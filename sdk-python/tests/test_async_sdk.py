...
@pytest.mark.asyncio
async def test_async_on_unauthorized_retries_once() -> None:
    """Mirrors the sync client: a 401 never ran the handler, so retrying with a
    replacement credential repeats no side effect."""
    seen: list[str | None] = []
    ...
    async with AsyncSferenceClient(
    ...
    assert me["username"] == "dev"
    assert seen == ["Bearer stale", "Bearer fresh"]


@pytest.mark.asyncio
async def test_async_on_unauthorized_returning_none_surfaces_401() -> None:
    calls: list[int] = []
