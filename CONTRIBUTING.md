...
Maintainers update it from the **sference-platform** private monorepo: run `just openapi` there (which regenerates `apps/api/openapi.json` and copies into `oss/contract/openapi.json` when the `oss` submodule is checked out). Then commit the change in the `sference` repository and bump the submodule pointer in `sference-platform`.
